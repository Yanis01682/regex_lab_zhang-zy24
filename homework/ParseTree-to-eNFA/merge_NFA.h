#ifndef REGEX_LAB_MERGE_NFA_H
#define REGEX_LAB_MERGE_NFA_H
...
NFA<T> merge_NFA(const NFA<T> &n1, const NFA<T> &n2,
                 std::size_t addition, typename NFA<T>::state **a,
    ...
    for (std::size_t i = 0; i < addition; ++i) {
        a[i] = &nfa.buf[i];
        nfa.Q.insert(a[i]);
    }
    ...
    for (std::size_t i = 0; i < n1.Q.size(); ++i) {
    ...
    for (std::size_t i = 0; i < n2.Q.size(); ++i) {
